...
namespace ZachLib.Statistics
{
    ...
    public class EigenvalueDecomposition
    {
        ...
        public KeyValuePair<int, double>[] PotentialLValues { get; private set; }
        ...
        /// <param name="mat">   Square matrix
        /// </param>
        /// <returns>     Structure to access D and V.
        /// </returns>

        public EigenvalueDecomposition(Matrix mat) : this(mat, mat.CheckSymmetry())
        {
        }

        public EigenvalueDecomposition(Matrix mat, bool isSymmetrical)
        {
            scaled = mat;
            issymmetric = isSymmetrical;
            n = mat.NumCols;
            d = new double[n];
            e = new double[n];
            ...
            double max = g[n - 1];
            PotentialLValues = new KeyValuePair<int, double>[n];
            for (int L = 1; L <= n; ++L)
            {
                PotentialLValues[L - 1] = new KeyValuePair<int, double>(L, g[L - 1] / max);
            }
        }
        ...
        public virtual double[][] GetV() => V;

        public virtual KeyValuePair<int[], Matrix> GetTransformedData(int L)
        {
            int[] selectedFeatures = new int[L];
            Matrix transformation = new Matrix(L, n);
            for (int k = 0; k < L; ++k)
            {
                int index = Array.IndexOf<double>(d, sorted[k]);
                selectedFeatures[k] = index;
                transformation[k] = V[index];
            }
            return new KeyValuePair<int[], Matrix>(selectedFeatures, transformation.Dot(scaled.T()).T());
        }
